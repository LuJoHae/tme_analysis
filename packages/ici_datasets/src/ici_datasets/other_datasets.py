...
class HarmonizedTcgaIAtlas(datalair.Dataset):
    """Datalair Dataset for the globally intersected and optionally ComBat-corrected TCGA and iAtlas datasets."""

    def __init__(self) -> None:
        super().__init__(namespace="HarmonizedTcgaIAtlas")

    def derive(self, lair: datalair.Lair) -> None:
        raise NotImplementedError(
            "This dataset cannot be automatically derived. It is generated by running `scripts/TCGA-background.py` "
            "which populates this path with the harmonized `.h5ad` files."
        )


class ImmunoCompassSignatures(datalair.Dataset):
    """Datalair Dataset for ImmunoCompass gene signatures."""

    def __init__(self) -> None:
        super().__init__(namespace="ImmunoCompassSignatures")
    ...
    def derive(self, lair: datalair.Lair) -> None:
        ...
        # 1. Download concepts_defined.txt
        download_url = "https://www.immuno-compass.com/download/other/concepts_defined.txt"
        dest_file = output_dir.joinpath("concepts_defined.txt")
        download(download_url, dest_file)
        
        # 2. Extract and format to standard GMT file
        df = pd.read_csv(dest_file, sep="\t")
        gmt_path = output_dir.joinpath("gene_signatures.gmt")
        with open(gmt_path, "w") as f:
            for _, row in df.iterrows():
                name = str(row['GeneSet']).strip()
                descr = str(row['Description']).strip()
                genes_str = str(row['Genes']).strip()
                genes = [g.strip() for g in genes_str.split(':') if g.strip()]
                if name and genes:
                    f.write(f"{name}\t{descr}\t" + "\t".join(genes) + "\n")
