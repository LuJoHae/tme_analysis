"""Preprocessing pipeline for GSE123139 single-cell melanoma AnnData dataset.

Checks normalization/log-transform status, applies TPM/total count normalization & log1p
if necessary, performs HVG identification (without subsetting), PCA on HVGs, KNN, Leiden,
and UMAP embedding.
"""
...
class PreprocessConfig(BaseModel):
    """Immutable configuration for GSE123139 Scanpy preprocessing."""
...
def normalize_and_log_transform(adata: ad.AnnData, target_sum: float) -> Result[ad.AnnData, str]:
    """Detect matrix normalization & log-transform status, applying CPM/TPM & log1p if needed."""
    try:
        X = adata.X
        ...
        # Check if already log1p transformed (typically max_val < 25.0 and non-integer)
...
def compute_hvg_pca_umap(adata: ad.AnnData, leiden_res: float) -> Result[ad.AnnData, str]:
    ...
    try:
        # Filter low-quality cells/genes before HVG calculation
...
def run_pipeline(config: PreprocessConfig) -> Result[Path, str]:
    """Monadic processing pipeline for GSE123139 scanpy preprocessing."""
...
def main() -> None:
    """CLI entry point for GSE123139 Scanpy preprocessing."""
    parser = argparse.ArgumentParser(description="Preprocess GSE123139 dataset (TPM/log1p, HVG, PCA, KNN, Leiden, UMAP).")
    ...
    match run_pipeline(config):
        case Success(out_path):
            print(f"Successfully finished preprocessing GSE123139 dataset: {out_path}")
            sys.exit(0)
        case Failure(err):
