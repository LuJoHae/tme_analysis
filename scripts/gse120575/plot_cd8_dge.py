...
def plot_volcano(csv_path: Path, out_dir: Path) -> None:
    ...
    plt.title("Responder vs Non-Responder DGE (CD8+ T cells)")
    plt.xlabel("Log2 Fold Change")
    plt.ylabel("-Log10(FDR)")
    plt.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig(out_dir / "cd8_responder_dge_volcano.svg", format="svg")
    plt.close()

def plot_dotplot(adata_path: Path, csv_path: Path, out_dir: Path) -> None:
    ...
    if genes:
        ...
        plt.savefig(out_dir / "cd8_responder_dge_dotplot.svg", bbox_inches='tight')
        plt.close()

def main() -> None:
    parser = argparse.ArgumentParser(description="Plot DEGs between Responder and Non-responder CD8+ T cells")
    ...
    plot_volcano(Path(args.csv), out_dir)
    
    print("Plotting DotPlot...")
    plot_dotplot(Path(args.adata), Path(args.csv), out_dir)
