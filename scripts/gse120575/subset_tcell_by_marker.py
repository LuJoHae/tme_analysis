...
def subset_tcells(adata_path: Path, out_path: Path, gene: str) -> None:
    ...
    print(f"\n--- Clusters KEEP (Mean {gene} > 0.5) ---")
    for c in keep_clusters:
        print(f"  Cluster {c}: {means[c]:.3f}")
        
    print(f"\n--- Clusters DROP (Mean {gene} <= 0.5) ---")
    ...
    sc.tl.umap(subset, min_dist=0.01, spread=2.0)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Subset T cells based on a marker gene expression")
    parser.add_argument("--adata", required=True, help="Input T-cell h5ad path")
    parser.add_argument("--out-adata", required=True, help="Output subset h5ad path")
    parser.add_argument("--marker", required=True, help="Marker gene to subset on (e.g., CD8A or CD4)")
    args = parser.parse_args()
    
    subset_tcells(Path(args.adata), Path(args.out_adata), args.marker)
