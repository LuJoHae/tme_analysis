...
def main():
    ...
    sns.scatterplot(x=X[:, 0], y=X[:, 1], hue=pred_labels, palette="tab10", alpha=0.8, s=60, edgecolor='k')
    plt.scatter(centers[:, 0], centers[:, 1], color='black', marker='X', s=250, label='K-Means Centers', zorder=10)
    plt.title(f"K-Means Clustering (True Mean Difference = {args.mean_diff})")
    plt.xlabel("$X_1$")
    plt.ylabel("$X_2$")
    plt.legend()
    plt.tight_layout()
    ...
    for sim in tqdm(range(args.n_simulations), desc="Simulating Null"):
        ...
        X_sim, _ = generate_mixture_data(n_samples_comp1=75, n_samples_comp2=75, 
                                         mean_diff=0.0, sigma=1.0, random_state=sim)
